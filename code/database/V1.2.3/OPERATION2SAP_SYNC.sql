        ...
        INSERT INTO TT_SAP_SYNCHRONOUS_TMP1
          ...
          SELECT D.ID,
            ...
            FROM TT_PB_SHEDULE_BY_DAY   D,
                 ...
                 --取确认的月表信息
                 (SELECT MAX(L.ID) LID, L.EMP_CODE, L.YM, L.DEPT_ID
                    FROM TT_PB_SHEDULE_BY_MONTH_LOG L
                   GROUP BY L.EMP_CODE, L.YM, L.DEPT_ID) ML,
                 TM_PB_SCHEDULE_BASE_INFO B,
                 TM_OSS_EMPLOYEE E
          --关联月表
           WHERE D.SHEDULE_MON_ID = M.ID
                --关联月确认表
             AND D.EMP_CODE = ML.EMP_CODE
             AND D.DEPT_ID = ML.DEPT_ID
             AND M.YM = ML.YM
                 --关联雇员表
            ...
            -- AND (E.TRANSFER_DATE IS NULL OR TO_CHAR(D.SHEDULE_DT, 'YYYYMMDD') >= TO_CHAR(E.TRANSFER_DATE, 'YYYYMMDD'))
             ...
             AND D.SHEDULE_DT >= DATE'2014-12-01'
                --取当天前的数据
