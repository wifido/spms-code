  ...
  IF KEYVALUE = '1' THEN
    ...
    BEGIN
      ...
      UPDATE TM_OSS_EMPLOYEE E
         SET E.EMP_POST_TYPE = 0
       WHERE EXISTS (SELECT 1
                FROM TI_SAP_SYNCHRONOUS_EMP EMP
               WHERE EMP.DEAL_FLAG = 0
                 AND EMP.IS_ODO_CJ IS NOT NULL
                 AND EMP.EMP_NUM = E.EMP_CODE)
         AND E.CANCEL_FLAG IS NULL
         AND E.DATA_SOURCE = '2'
         AND E.EMP_POST_TYPE = 2;
    
      update ti_sap_synchronous_emp t
         set t.deal_flag = '1'
       where t.IS_ODO_CJ is not null;
      commit;
      ...
      INSERT INTO TM_EMP_DEPT_CHANGE_RECORD
        (ID, EMP_CODE, LAST_ZNO, CURRENT_ZNO, CHANGE_DATE)
        SELECT SEQ_EMP_DEPT_CHANGE_RECORD.NEXTVAL,
               EMP_CODE,
               LAST_ZNO,
               CURRENT_ZNO,
               DATE_FROM
          FROM (SELECT TMP.EMP_CODE,
                       D.DEPT_CODE   LAST_ZNO,
                       TMP.NET_CODE  CURRENT_ZNO,
                       TMP.DATE_FROM DATE_FROM
                  FROM TM_OSS_EMPLOYEE_TMP TMP,
                       TM_OSS_EMPLOYEE     E,
                       TM_DEPARTMENT       D
                 WHERE TMP.EMP_CODE = E.EMP_CODE
                   AND E.DEPT_ID = D.DEPT_ID
                   AND TMP.DEPT_ID <> E.DEPT_ID);
