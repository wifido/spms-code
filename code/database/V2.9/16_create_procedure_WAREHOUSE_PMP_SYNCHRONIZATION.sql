CREATE OR REPLACE PROCEDURE WAREHOUSE_PMP_SYNCHRONIZATION IS
  /**
...
BEGIN
  ...
  IF KEYVALUE = '1' THEN
    ...
    ----推送SAP数据--begin
    --1.插入临时数据
    INSERT INTO TT_PMP_SYNCHRONOUS_TMP3
      (ID,
       EMP_CODE,
       BEGIN_DATE,
       END_DATE,
      ...
      AND (E.TRANSFER_DATE is null or D.DAY_OF_MONTH >= TO_CHAR(E.TRANSFER_DATE, 'YYYYMMDD'))
           --过滤转岗前的数据
      AND  D.DAY_OF_MONTH >= TO_CHAR(E.Sf_Date, 'YYYYMMDD')
         ...
         AND D.DAY_OF_MONTH >= '20150101'
            -- 取40天以内的数据
         AND D.DAY_OF_MONTH >= to_char(TRUNC(SYSDATE - 40),'YYYYMMDD')
    ...
    INSERT INTO TT_PMP_SYNCHRONOUS
      ...
      SELECT SEQ_TT_PMP_SYNCHRONOUS.NEXTVAL,
             ...
             S.ID,
             S.EMP_POST_TYPE
        FROM TT_PMP_SYNCHRONOUS_TMP3 S
       WHERE S.TMR_DAY_FLAG IS NULL;
  --跨天处理,拆分数据
  FOR SAP_ROW IN (SELECT * FROM TT_PMP_SYNCHRONOUS_TMP3 S
   WHERE S.TMR_DAY_FLAG = 'X') LOOP
      ...
      INSERT INTO TT_PMP_SYNCHRONOUS
          (ID,
           ...
           EMP_POST_TYPE)
        VALUES
          (SEQ_TT_PMP_SYNCHRONOUS.NEXTVAL,
           SAP_ROW.EMP_CODE，
           SAP_ROW.BEGIN_DATE,
           SAP_ROW.BEGIN_DATE,
           SAP_ROW.BEGIN_TM,
    ...
    UPDATE TT_SCHEDULE_DAILY D
     ...
     WHERE D.ID IN
           (SELECT ID FROM TT_PMP_SYNCHRONOUS_TMP3 E)
       AND EMP_POST_TYPE = '3';
    COMMIT;
    EXECUTE IMMEDIATE 'TRUNCATE TABLE TT_PMP_SYNCHRONOUS_TMP3';
    ----推送SAP数据--end
    END;
  END IF;
  --4.结束记录日志
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    PKG_OSS_COMM.STP_RUNNING_LOG('',
                                 ...
                                 0,
                                 L_CALL_NO);
END WAREHOUSE_PMP_SYNCHRONIZATION;
/