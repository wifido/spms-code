CREATE OR REPLACE PROCEDURE OPERATION_PMP_SYNC AS
  /**
  ...
  BEGIN
    ...
    IF KEYVALUE = '1' THEN
      BEGIN
        ...
        --a.插入接口数据至临时表
        INSERT INTO TT_PMP_SYNCHRONOUS_TMP1
          (ID,
           EMP_CODE,
           BEGIN_DATE,
           END_DATE,
        ...
        INSERT INTO TT_PMP_SYNCHRONOUS
          ...
          SELECT SEQ_TT_PMP_SYNCHRONOUS.NEXTVAL,
                 ...
                 TMP.ID,
                 TMP.EMP_POST_TYPE
            FROM TT_PMP_SYNCHRONOUS_TMP1 TMP
        ...
        WHERE D.ID IN (SELECT ID FROM TT_PMP_SYNCHRONOUS_TMP1 E);
        COMMIT;
        --f.删除临时表
        EXECUTE IMMEDIATE 'TRUNCATE TABLE TT_PMP_SYNCHRONOUS_TMP1';
        ----推送运作排班数据至SAP接口表--END
      END;
    END IF;
  END;
