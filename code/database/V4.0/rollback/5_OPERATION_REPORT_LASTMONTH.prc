...
EXECUTE IMMEDIATE 'TRUNCATE TABLE TI_TCAS_SPMS_tmp1';
  ...
  commit;
   ...
   -- 循环上个月每一天
   FOR I IN 0 ..  LAST_DAY(ADD_MONTHS(SYSDATE, -1))  - (LAST_DAY(ADD_MONTHS(SYSDATE, -2)) + 1) LOOP

     SELECT TO_CHAR(LAST_DAY(ADD_MONTHS(SYSDATE, -2)) + 1 + I ,'YYYY-MM-DD')  INTO DAYOFMONTH  FROM DUAL;
