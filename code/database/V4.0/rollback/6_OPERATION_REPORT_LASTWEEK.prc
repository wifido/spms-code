CREATE OR REPLACE PROCEDURE OPERATION_REPORT_LASTWEEK IS
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'TRUNCATE TABLE TI_TCAS_SPMS_tmp4';

  insert into TI_TCAS_SPMS_tmp4
  select * from TI_TCAS_SPMS_SCHEDULE t
  where t.work_date >= trunc(sysdate - 8)
  and t.work_date <= sysdate
    ...
    FOR DATA_ROW IN (SELECT *
     ...
     V_TOTAL_ATTENDANCE_NUM        := 0;
      ...
      SELECT NVL(SUM(T.KQ_XSS), 0)
        ...
        FROM TI_TCAS_SPMS_tmp4 T, TT_EMPLOYEE_RECORD_HIS4 HI
    ...
    INSERT INTO OP_ATTENDANCE_COUNT_REPORT
      SELECT DECODE(DEPT.AREA_CODE,
       ...
       WHERE T.DEPT_CODE = DEPT.DEPT_CODE
         AND T.DAY_OF_MONTH = to_date(DAYOFMONTH, 'YYYY-MM-DD')
