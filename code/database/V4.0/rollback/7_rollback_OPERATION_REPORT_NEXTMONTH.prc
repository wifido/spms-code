CREATE OR REPLACE PROCEDURE OPERATION_REPORT_NEXTMONTH IS
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE TI_TCAS_SPMS_tmp2';

 insert into TI_TCAS_SPMS_tmp2
  select * from TI_TCAS_SPMS_SCHEDULE t
  where t.work_date >= trunc(LAST_DAY(ADD_MONTHS(SYSDATE,0)) + 1)
  ...
  commit;
   ...
   -- 循环历史一周每一天
   FOR I IN 0 .. LAST_DAY(ADD_MONTHS(SYSDATE, 1))  - (LAST_DAY(ADD_MONTHS(SYSDATE,0)) + 1) LOOP

     SELECT TO_CHAR(LAST_DAY(ADD_MONTHS(SYSDATE,0)) + 1 + I ,'YYYY-MM-DD')   INTO DAYOFMONTH  FROM DUAL;
    ...
    FOR DATA_ROW IN (SELECT *
     ...
     V_TOTAL_ATTENDANCE_NUM        := 0;
      ...
      DELETE TT_EMPLOYEE_RECORD_HIS2;

      INSERT INTO TT_EMPLOYEE_RECORD_HIS2
        SELECT T1.EMP_CODE,
               T1.DEPT_CODE,
               T2.POST_TYPE,
               T3.PERSG,
         ...
         WHERE T1.EMP_CODE = T2.EMP_CODE
           ...
           AND (T4.SF_DATE <= TO_DATE(DAYOFMONTH, DATE_FORMAT) OR T4.SF_DATE IS NULL)
      ...
      SELECT NVL(COUNT(1), 0)
        ...
        FROM TT_EMPLOYEE_RECORD_HIS2 T;
      ...
      SELECT NVL(COUNT(DISTINCT S.EMP_CODE), 0)
        ...
        FROM TT_EMPLOYEE_RECORD_HIS2 D,
      ...
      SELECT NVL(SUM(T.KQ_XSS), 0)
        ...
        FROM TI_TCAS_SPMS_tmp2 T, TT_EMPLOYEE_RECORD_HIS2 HI
      ...
      SELECT NVL(COUNT(*), 0)
        ...
        FROM TT_EMPLOYEE_RECORD_HIS2 E, TT_PB_SHEDULE_BY_DAY T
      ...
      SELECT NVL(COUNT(*), 0)
        ...
        FROM TT_EMPLOYEE_RECORD_HIS2 E,
...
END OPERATION_REPORT_NEXTMONTH;
/
