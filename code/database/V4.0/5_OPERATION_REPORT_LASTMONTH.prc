CREATE OR REPLACE PROCEDURE OPERATION_REPORT_LASTMONTH IS
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'TRUNCATE TABLE TI_TCAS_SPMS_tmp1';

  insert into TI_TCAS_SPMS_tmp1
    select *
      from TI_TCAS_SPMS_SCHEDULE t
     where t.work_date >= trunc(LAST_DAY(ADD_MONTHS(SYSDATE, -2)) + 1)
       and t.work_date <= trunc(LAST_DAY(ADD_MONTHS(SYSDATE, -1)))
       and t.position_type = '1';
  commit;

  -- 循环上个月每一天
  FOR I IN 0 .. LAST_DAY(ADD_MONTHS(SYSDATE, -1)) -
                (LAST_DAY(ADD_MONTHS(SYSDATE, -2)) + 1) LOOP

    SELECT TO_CHAR(LAST_DAY(ADD_MONTHS(SYSDATE, -2)) + 1 + I, 'YYYY-MM-DD')
      INTO DAYOFMONTH
      FROM DUAL;
    ...
    FOR DATA_ROW IN (SELECT * FROM OP_DEPT T) LOOP
      ----初始化值
      V_TOTAL_EMP_NUM               := 0;
      ...
      DELETE TT_EMPLOYEE_RECORD_HIS1;

      INSERT INTO TT_EMPLOYEE_RECORD_HIS1
        SELECT T1.EMP_CODE,
               T1.DEPT_CODE,
               T2.POST_TYPE,
               T3.PERSG,
      ...
      SELECT NVL(COUNT(1), 0)
        ...
        FROM TT_EMPLOYEE_RECORD_HIS1 T;
      ...
      SELECT NVL(COUNT(DISTINCT S.EMP_CODE), 0)
        ...
        FROM TT_EMPLOYEE_RECORD_HIS1 D,
      ...
      SELECT NVL(SUM(T.KQ_XSS), 0)
        ...
        FROM TI_TCAS_SPMS_tmp1 T, TT_EMPLOYEE_RECORD_HIS1 HI
      ...
      SELECT NVL(COUNT(*), 0)
        ...
        FROM TT_EMPLOYEE_RECORD_HIS1 E, TT_PB_SHEDULE_BY_DAY T
      ...
      SELECT NVL(COUNT(*), 0)
        ...
        FROM TT_EMPLOYEE_RECORD_HIS1 E,
...
END;
--4 结束记录日志
PKG_OSS_COMM.STP_RUNNING_LOG('', 'OPERATION_REPORT_LASTMONTH', SYSDATE, NULL, NULL, 'END', 0, L_CALL_NO);
EXCEPTION
WHEN OTHERS THEN
--回滚数据
ROLLBACK; PKG_OSS_COMM.STP_RUNNING_LOG('', 'OPERATION_REPORT_LASTMONTH', SYSDATE, SQLCODE, SQLERRM, 'ERROR', 0, L_CALL_NO);

END OPERATION_REPORT_LASTMONTH;
/
