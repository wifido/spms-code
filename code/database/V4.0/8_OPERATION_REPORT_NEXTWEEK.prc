CREATE OR REPLACE PROCEDURE OPERATION_REPORT_NEXTWEEK IS
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'TRUNCATE TABLE TI_TCAS_SPMS_tmp3';

  insert into TI_TCAS_SPMS_tmp3
  select * from TI_TCAS_SPMS_SCHEDULE t
  where t.work_date >= trunc(sysdate)
  and t.work_date <= sysdate + 6
  ...
  commit;
   ...
   FOR I IN 0 .. 6 LOOP
    ...
    DELETE OP_ATTENDANCE_COUNT_REPORT T
     WHERE T.DAY_OF_MONTH = TO_DATE(DAYOFMONTH, DATE_FORMAT);
     COMMIT;
    ...
    FOR DATA_ROW IN (SELECT *
     ...
     V_TOTAL_ATTENDANCE_NUM        := 0;
      ...
      DELETE TT_EMPLOYEE_RECORD_HIS3;

      INSERT INTO TT_EMPLOYEE_RECORD_HIS3
        SELECT T1.EMP_CODE,
               T1.DEPT_CODE,
               T2.POST_TYPE,
               T3.PERSG,
      ...
      SELECT NVL(COUNT(1), 0)
        ...
        FROM TT_EMPLOYEE_RECORD_HIS3 T;
      ...
      SELECT NVL(COUNT(DISTINCT C.SCHEDULE_CODE), 0)
       ...
       WHERE S.DEPT_ID = D.DEPT_ID
         ...
         AND S.SHEDULE_DT = TO_DATE(DAYOFMONTH, DATE_FORMAT);
      ...
      SELECT NVL(COUNT(DISTINCT T.EMP_CODE), 0)
        ...
        FROM TM_DEPARTMENT          D,
             TI_TCAS_SPMS_tmp3  T,
             TT_EMPLOYEE_RECORD_HIS3 E
       WHERE T.DEPT_CODE = D.DEPT_CODE
         AND T.EMP_CODE = E.EMP_CODE
         AND T.POSITION_TYPE = 1
         AND T.WORK_DATE = TO_DATE(DAYOFMONTH, DATE_FORMAT)
         AND T.WORK_TIME <> 0
         AND D.DEPT_CODE = DATA_ROW.DEPT_CODE;
      ...
      SELECT NVL(COUNT(DISTINCT S.EMP_CODE), 0)
        ...
        FROM TT_EMPLOYEE_RECORD_HIS3 D,
      ...
      SELECT NVL(SUM(T.KQ_XSS), 0)
        ...
        FROM TI_TCAS_SPMS_tmp3 T, TT_EMPLOYEE_RECORD_HIS3 HI
      ...
      SELECT NVL(COUNT(*), 0)
        ...
        FROM TT_EMPLOYEE_RECORD_HIS3 E, TT_PB_SHEDULE_BY_DAY T
      ...
      SELECT NVL(COUNT(*), 0)
        ...
        FROM TT_EMPLOYEE_RECORD_HIS3 E,
