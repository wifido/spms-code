CREATE OR REPLACE PACKAGE BODY PKG_HANDLE_COMPARE_REPORT IS
  ...
  PROCEDURE P_REPORT_CAUSE_BY_DRIVING IS
    ...
    L_LINE_ID VARCHAR2(21);
    ----排班表中的配班代码;
    L_CONFIGURE_CODE VARCHAR2(21);
    --- 排班对应的年月.如2015-09
    L_YEAR_MONTH VARCHAR2(21);
  ...
  BEGIN
    ...
    -- 行车日志数据只能取状态为0(还未对比处理的数据)
    FOR V_TEMP IN (SELECT *
                     FROM ti_driver_log_tmp
                    WHERE COMPARE_STATUS = 0
      ...
      -- 计算被拼接的线路中的条数;
      IF V_TEMP.DEPT_CODE IS NULL THEN
        L_DRIVING_LINE_COUNT := 0;
      ELSE
        SELECT LENGTH(V_TEMP.DEPT_CODE) -
               LENGTH(REPLACE(V_TEMP.DEPT_CODE, '-'))
          INTO L_DRIVING_LINE_COUNT
          FROM DUAL;
      END IF;
    
      ---根据行车日志ID,获取到排班数据;
      SELECT MAX(T.CONFIGURE_CODE), MAX(T.YEAR_MONTH)
        INTO L_CONFIGURE_CODE, L_YEAR_MONTH
        FROM TT_DRIVER_SCHEDULING T
       WHERE T.EMPLOYEE_CODE = V_TEMP.DRIVE_MEMBER
         AND DAY_OF_MONTH = V_TEMP.DAY_OF_MONTH
         AND SCHEDULING_TYPE = 1;
    
      --如果查询不到对应天的配班代码,表示还未排班;
      IF L_CONFIGURE_CODE IS NULL THEN
        L_SCHEDULE_LINE_COUNT := 0;
      ELSE
        SELECT COUNT(*)
          INTO L_SCHEDULE_LINE_COUNT
          FROM TT_DRIVER_LINE_CONFIGURE T, TT_DRIVER_LINE_CONFIGURE_R TT
         WHERE DEPARTMENT_CODE =
               substr(L_CONFIGURE_CODE, 0, instr(L_CONFIGURE_CODE, '-') - 1)
           AND CODE =
               substr(L_CONFIGURE_CODE, instr(L_CONFIGURE_CODE, '-') + 1)
           AND T.ID = TT.CONFIGURE_ID
           AND T.MONTH = L_YEAR_MONTH;
      END IF;
      -------------------------------------------------------------------------------------------------------------------------------
      -- 3.判断最终对比结果;
      ...
      **/
      ...
      IF L_SCHEDULE_LINE_COUNT <> 0 THEN
        ...
        ELSE
          IF L_SCHEDULE_LINE_COUNT = L_DRIVING_LINE_COUNT THEN
            L_FINAL_RESULT := 0;
          ELSIF L_SCHEDULE_LINE_COUNT > L_DRIVING_LINE_COUNT THEN
            L_FINAL_RESULT := 3;
          ELSIF L_SCHEDULE_LINE_COUNT < L_DRIVING_LINE_COUNT THEN
            L_FINAL_RESULT := 4;
          END IF;
      ...
      --查询出网点对应的名称,区部名称;
      SELECT MAX(B.DEPT_CODE),
             MAX(B.DEPT_NAME),
             MAX(C.DEPT_CODE),
             MAX(C.DEPT_NAME),
             MAX(A.EMP_NAME)
        INTO L_DEPT_CODE, L_DEPT_NAME, L_AREA_CODE, L_AREA_NAME, L_EMP_NAME
        FROM TM_OSS_EMPLOYEE A, TM_DEPARTMENT B, TM_DEPARTMENT C
       WHERE A.DEPT_ID = B.DEPT_ID
         AND B.AREA_CODE = C.DEPT_CODE
         AND A.EMP_CODE = V_TEMP.DRIVE_MEMBER;
    
      SELECT COUNT(*)
        INTO EXIST_REPORT
        FROM TT_COMPARED_REPORT
       WHERE EMPLOYEE_CODE = V_TEMP.DRIVE_MEMBER
         AND DAY_MONTH = V_TEMP.DAY_OF_MONTH;
      ...
      FOR TEMP IN (select t.drive_member,
        ...
        FOR DRIVER_LOG_ROW IN (select *
          ...
          if TEMP_JOIN_DEPARTMENT_CODE is not null then
            if length(TEMP_JOIN_DEPARTMENT_CODE) < 1900 then
