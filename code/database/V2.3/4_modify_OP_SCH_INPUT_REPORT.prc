create or replace procedure OP_SCH_INPUT_REPORT(YEAR_MONTH in varchar2) is
  ...
  sch_confirm_diurnal_number     NUMBER;
  sch_confirm_non_diurnal_number NUMBER;
...
begin
  ...
  for dept_row in (select *
    ...
    scheduled_complete_rate  := 0;
    sch_confirm_diurnal_number     := 0;
    sch_confirm_non_diurnal_number := 0;
    ...
    -- 排班确认“全日制用工”总数
    select count(dept.dept_code) sche_confirm_inner_emp_num
      into sch_confirm_diurnal_number
      from op_dept dept,
           (select dept.dept_code, record.ym, emp.emp_code
              from op_dept dept,
                   tt_pb_shedule_by_month_log record,
    ...
    -- 排班确认“非全日制用工”总数
    select count(dept.dept_code) sche_confirm_inner_emp_num
      into sch_confirm_non_diurnal_number
      from op_dept dept,
           (select dept.dept_code, record.ym, emp.emp_code
              from op_dept dept,
                   tt_pb_shedule_by_month_log record,
                   (select emp.emp_code, emp.dept_id, emp.dimission_dt, emp.persg_txt
                      from tm_oss_employee emp
                     where emp.emp_post_type = 1
                       and emp.data_source = '2'
                       and (emp.dimission_dt is null or
    ...
    values
      (seq_op_sch_statis_report.nextval,
       ...
       diurnal_number,
       non_diurnal_number,
       sch_confirm_diurnal_number,
       sch_confirm_non_diurnal_number);
