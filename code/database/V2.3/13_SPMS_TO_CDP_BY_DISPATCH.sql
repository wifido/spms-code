  ...
  INSERT INTO TT_CDP_EMPNUM_SCHEDULINGNUM
    ...
    TC AS
     (SELECT A.DEPT_CODE, COUNT(a.EMP_CODE) P_NUM_Q
       ...
       WHERE PLA.DUTYDATE = to_char(SYSDATE + V_COUNT, 'yyyy-mm-dd')
       GROUP by A.DEPT_CODE)
    SELECT TO_CHAR(SYSDATE + V_COUNT, 'YYYYMMDD'),
           ...
           nvl(NFTSCNUM.g_NUM_F, 0) + NVL(FTSCNTC.P_NUM_Q, 0) +
           nvl(TC.P_NUM_Q, 0),
           '一线',
           SYSDATE,
           2
      ...
      LEFT JOIN FTSCNTC
        ON NUMberTime.DEPT_CODE = FTSCNTC.DEPT_CODE
      LEFT JOIN TC
        on NUMberTime.dept_code = TC.dept_code;
  COMMIT;
