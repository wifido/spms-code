CREATE OR REPLACE PROCEDURE DISPATCH_MONITOR_MONTH IS
  --*************************************************************
  -- AUTHOR  : JHC
  -- CREATED : 2016-03-07
  -- PURPOSE : 一线监控表
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE TT_SCHEDULE_DAILY_MONTH';

INSERT INTO TT_SCHEDULE_DAILY_MONTH
  SELECT *
    FROM TT_SCHEDULE_DAILY T
   WHERE T.EMP_POST_TYPE = '2'
     AND T.MONTH_ID =
         TO_CHAR(ADD_MONTHS(TRUNC(SYSDATE, 'MONTH'), -1), 'YYYYMM');
         COMMIT;


for V_COUNT IN (SELECT ADD_MONTHS(TRUNC(SYSDATE, 'MONTH'), -1) + (ROWNUM - 1) S_DATE
  FROM DUAL
CONNECT BY ROWNUM <= LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE, 'MONTH'), -1)) -
           ADD_MONTHS(TRUNC(SYSDATE, 'MONTH'), -1) + 1) LOOP

delete TT_DISPATCH_MONITOR_REPORT t where t.day_of_month = V_COUNT.S_DATE AND t.EMP_POST_TYPE ='2';
  ...
  QRZPBRS AS
   (SELECT DA.DEPARTMENT_CODE DEPT_CODE,
      ...
      FROM TT_SCHEDULE_DAILY_MONTH DA
      JOIN A
