// <%@ page language="java" contentType="text/html; charset=utf-8"%>
	...
	function queryApprover(searchString) {
		...
		$.ajax({
			...
			success : function(resp) {
				...
				var result = jQuery.parseJSON(resp.resultMap.root);
				
				totalSize = result.resultMap.totalSize;
				...
				var html = '';
				$.each(result.resultMap.root,function(i, row){
		...
		if (typeof url == "string") {
			...
			if (url.indexOf(pageIdOfOptionApprover) != -1) {
				...
				if(params['newSchedulingCode']!= null) {
					...
					$('#exchangeSchedulingForm textarea[id=exchangeSchedulingApplyInfo]').val('');
				}
	...
	function loadLineConfigData(pageIndex, searchStr) {
		...
		$.ajax({
			...
			success : function(data, textStatus) {
				$.mobile.hidePageLoadingMsg();
				var result = jQuery.parseJSON(data.resultMap.root);
				var totalPage = parseInt($("#totalPage").val());
				...
				$.each(result.resultMap.root, function(i, val) {
					html += '<li><a href="#optionApprover?newSchedulingCode=' + formatLineConfiguerCode(val) + '&employeeCode='+$('#applyEmployeeCode').val()+'" data-rel="dialog">';
	...
	function loadData(year_week) {
		...
		$.getJSON("myScheduling.action", {
			...
			if (result.resultMap.root.length < 1) {
	...
	function confirmScheduling() {
		...
		$.getJSON("confirmScheduling.action", {
			...
			$.mobile.hidePageLoadingMsg();
			var result = jQuery.parseJSON(json.resultMap.root);
			if ('success' == result.resultMap.root) {
	...
	function setApproverParams(dayOfMonth, departmentCode, configureCode) {
		...
		$('#notification').text("");
		$('#leaveForm textarea[id=applyInfo]').val('');
	...
	function buildLeaveButton(scheduling) {
		return '<a href="#optionApprover" onclick="setApproverParams(\'' + scheduling.dayOfMonth + '\',\''
			...
			+ scheduling.dayOfMonth + '" data-rel="dialog" data-mini="true"  data-theme="x" data-role="button" data-inline="true">请假</a>';
	}
	...
	function buildRevokeLeaveButton(scheduling) {
		return '<a href="#" onclick="setRevokeLeaveFormValue(' + scheduling.dayOfMonth + ',\''+scheduling.employeeCode+'\')" class="revokeLeave_' + scheduling.dayOfMonth
				+ '"  data-role="button" data-inline="true"  data-theme="x" data-mini="true" >撤销请假</a>';
	}
	...
	return '<a href="#selectConfigCode?oldSchedulingCode='
				...
				+ scheduling.dayOfMonth
				+ '" data-rel="dialog" data-mini="true" data-role="button"   data-theme="x" data-inline="true">调班</a>';
	}
	...
	function buildRevokeExchangeScheduling(scheduling) {
		return '<a href="#" onclick="setRevokeExchangeSchedulingFormValue('
				...
				+ scheduling.dayOfMonth
				+ '" data-role="button" data-inline="true" data-mini="true"  data-theme="x" >撤回</a>';
	}
	...
	function revokeExchangeScheduling() {
		...
		$.ajax({
				...
				success : function(data, textStatus) {
					...
					var dayOfMonth = $("#revokeExchangeSchedulingWindow input[name=dayOfMonth]").val();
					var btnLeave = $('.leave_' + dayOfMonth + '');
					var btnExchangeScheduling = $('.exchangeScheduling_'
							+ dayOfMonth + '');
					var btnRevokeExchangeScheduling = $('.revokeExchangeScheduling_'
					...
					if (result.resultMap.configureInfor.oldConfigCode != "休") {
