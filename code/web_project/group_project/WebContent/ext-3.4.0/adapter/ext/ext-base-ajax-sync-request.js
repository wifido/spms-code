...
(function() {
	...
	// Safari
	// 2
	isSafari3 = isSafari && check(/version\/3/), isSafari4 = isSafari && check(/version\/4/), isIE = !isOpera && check(/msie/), isIE7 = isIE && (check(/msie 7/) || docMode == 7), isIE8 = isIE
			&& (check(/msie 8/) && docMode != 7), isIE9 = isIE && check(/msie 9/), isIE6 = isIE && !isIE7 && !isIE8 && !isIE9, isGecko = !isWebKit && check(/gecko/), isGecko2 = isGecko
			&& check(/rv:1\.8/), isGecko3 = isGecko && check(/rv:1\.9/), isBorderBox = isIE && !isStrict, isWindows = check(/windows|win32/), isMac = check(/macintosh|mac os x/), isAir = check(/adobeair/), isLinux = check(/linux/), isSecure = /^https/i
					...
					.apply(
							EXTLIB.Motion.prototype,
							{
								...
								doMethod: function(attr, start, end) {
									...
									return pointsRe.test(attr) ? EXTLIB.Bezier.getPosition(me.runAttrs[attr], me.method(me.curFrame, 0, 100, me.totalFrames) / 100) : superclass.doMethod.call(me,
											attr, start, end);
								},
								...
								setRunAttr: function(attr) {
									if (pointsRe.test(attr)) {
										...
										if (control.length > 0 && !Ext.isArray(control[0])) {
											...
											/*
											 * var tmp = []; for (i = 0,len =
											 * control.length; i < len; ++i) {
											 * tmp[i] = control[i]; } control =
											 * tmp;
											 */
										}
