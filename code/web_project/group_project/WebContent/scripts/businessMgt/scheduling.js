...
var uploadform = new Ext.form.FormPanel(
		{
			...
			items : [
					...
					new Ext.form.Label(
							{
								html : '<font style="margin-right:60px;font: 11px Verdana, Arial, Helvetica, sans-serif;">模板文件:</font><a href="downloadTempExcel.action">排班导入模板.xls</a>'
							}) ]
		});
							...
							Ext.MessageBox.wait('正在保存数据...');
								uploadform.getForm().doAction('submit', {
									...
									success : function(form, action){
										...
										confirm_Store.load({params:{start:0,limit:20}});
  		...
  		load :function(store, records){
			...
			Ext.each(records, function(res, index ) {
				...
				if (!Ext.isEmpty(dimissionDt) && dimissionDt.indexOf(ym) == 0) {
					...
					var colct = ddt.getDaysInMonth() + colSidx;
					var colIndex = ddt.getDate() + colSidx;
...
Ext.onReady(function() {
							...
							}),
							{
								...
								items:[{
									...
									store:new Ext.data.JsonStore({
								        ...
								        fields: [{name:'classCode',mapping:'SCHEDULE_CODE'},
										         {name:'classCode',mapping:'SCHEDULE_CODE'}],
								         listeners : {
												beforeload :function(s,o){
													s.setBaseParam("deptId",Ext.getCmp('q_deptId').getValue());
													s.setBaseParam("state",1);
												},
												load : function(s) {
													if (!s.getCount() || s.getAt(0).data.classCode != '休')
														s.insert(0, new s.recordType({classCode : '休', classCode :'休'}));
												}
											}
								    })
								}]
	...
	var btnConfirm = new Ext.Button({
		...
		handler : function(){
			...
			Ext.Ajax.request( {  
			    timeout : 3000,  
			    url : 'sechMgt_checkCanConfirm.action',  
			    ...
			    success : function(response, config) {  
			    	...
			    	if(res.confirmFlag){
			    		...
			    		Ext.Msg.alert('提示','该月份的排班还未完成，不能提交！');
			    	}
			    }  
			})
		}
	...
	var viewreprot = new Ext.Viewport({
		...
		items : [ treePanel, 
		          new Ext.Panel({
								...
								region : "center",
								tbar : ['-'
										<app:isPermission code="/scheduling/search.action">,btnSearch</app:isPermission>
										<app:isPermission code="/scheduling/add.action">,'-',btnAdd</app:isPermission>
										<app:isPermission code="/scheduling/edit.action">,'-',btnEdit</app:isPermission>
										<app:isPermission code="/scheduling/export.action">,'-',btnExport</app:isPermission>
										<app:isPermission code="/scheduling/import.action">,'-',btnImport</app:isPermission>
										<app:isPermission code="/scheduling/confirm.action">,'-',btnConfirm</app:isPermission>
								         ],
...
var confirm_Store = new Ext.data.Store({ 
	...
	[{name:'id',      mapping:'id'	    },
	 ...
	 {name:'errorDesc',mapping:'errorDesc'},
...
var confirm_sm = new Ext.grid.CheckboxSelectionModel({singleSelect:false});
var confirm_gridView = new Ext.grid.GridPanel({
	...
	cm : new Ext.grid.ColumnModel([ 
	    confirm_sm, 
	    {header:'描述',    dataIndex:'errorDesc',align:'center',width:120},
	    {header:'年月',    dataIndex:'ym',align:'center',width:90}, 
	    {header:'地区代码',dataIndex:'areaCode',align:'center',width:90},
	    {header:'网点代码',dataIndex:'deptCode',align:'center',width:90},
	    {header:'工号',    dataIndex:'empCode',align:'center',width:90}, 
