...
var downTemplate = function(){
	url = "uniquePart=6adf1b32-31f8-4125-91ec-b50ba39bc4d9admin&fileName=工序安排导入模板.xls&moduleName=operation&entityName=processMgt&isTemplate=true";
...
var uploadform = new Ext.form.FormPanel(
		{
			...
			items : [
					...
					new Ext.form.Label(
							{
								html : '<font style="margin-right:60px;font: 11px Verdana, Arial, Helvetica, sans-serif;">模板文件:</font> <a href="#" onclick="downTemplate()">排工序导入模板.xls</a>'
							}) ]
		});
									...
									success : function(form, action){
										if (Ext.isEmpty(action.result.msg)) {
											 ...
											 if(!Ext.isEmpty(action.result.fileName))
												 window.location = '../common/downloadReportFile.action?' + Ext.util.Format.htmlDecode(encodeURI(encodeURI(action.result.fileName)));
										  }else{
...
var gridView = new Ext.grid.GridPanel({
	...
	cm : new Ext.grid.ColumnModel([ 
	    ...
	    {header:'用工类型',   dataIndex:'workTypeStr',align:'center',width:90},
	    {
			header : '提交状态',
...
Ext.onReady(function() {
	...
	var treePanel = new Ext.tree.TreePanel({
		...
		listeners: {
            click: function(n) {
            	if(n.id != 0 && (filterDeptCodeType.indexOf(n.attributes.typeCode+',') != -1)){
...
//			});
	...
	var btnConfirmExport = new Ext.Button({
		...
		handler : function(){
			queryView.getForm().submit({
				url : "processManagement_confirmExport.action",
				timeout:15*60,
				waitTitle:'请稍后',
    	        waitMsg : '正在执行操作...',
				method : "post",
	...
	var btnDelete = new Ext.Button({
		...
		handler : function(){
			...
			var processMgtIds = new Array();
			for(var i = 0; i < records.length; i++) {
				processMgtIds.push(records[i].data.id);
			}
			Ext.Ajax.request( {  
			    timeout : 30000,  
			    url : 'processManagement_checkIsCanDelete.action',  
			    params:{'dto.deptId' : Ext.getCmp("q_deptId").getValue(),
			    		'processMgtIds' : processMgtIds.toString()},
			    success : function(response, config) { 
				    	var res  = Ext.decode(response.responseText); 
				    	if(!Ext.isEmpty(res.msg)){
				    		Ext.Msg.alert('提示',res.msg);
				    	...
				    	if(res.confirmFlag){
							Ext.Msg.confirm('提示','确认删除未确认的工序安排吗？',function(b){
								if("no"==b){
									return false;
								}else{
									Ext.Ajax.request( {  
									    timeout : 30000,  
									    url : 'processManagement_delete.action',
									    params:{'dto.deptId' : Ext.getCmp("q_deptId").getValue(),
									    	'processMgtIds' : processMgtIds.toString()},
									    success : function(response, config) {  
									    	var rep  = Ext.decode(response.responseText); 
		...
		items : [ treePanel, 
		          new Ext.Panel({
								...
								tbar : ['-'
										...
										<app:isPermission code="/processMgt/delete.action">,'-',btnDelete</app:isPermission>
										<app:isPermission code="/processMgt/confirmExport.action">,'-',btnConfirmExport</app:isPermission>
								         ],
