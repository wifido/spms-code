...
var topPanel = new Ext.Panel({
				...
				height : 160,
				tbar:['-'
				      <app:isPermission code="/schedulingBase/processMgt_queryProcess.action">,btnSearch</app:isPermission>
				      	,'-'
				      <app:isPermission code="/schedulingBase/processMgt_saveProcess.action">,btnAdd</app:isPermission>
				       , '-'
				      <app:isPermission code="/schedulingBase/processMgt_updateProcess.action">,btnEdit</app:isPermission>
				      	,'-'
				      <app:isPermission code="/schedulingBase/processMgt_confirmProcess.action">,btnProcess</app:isPermission>
				      	,'-'
				     <app:isPermission code="/schedulingBase/processUploadFile.action">,btnImport</app:isPermission>
				      	,'-'
				      <app:isPermission code="/schedulingBase/processMgt_exportProcess.action">,btnExport</app:isPermission>
				],
				items : [{
					xtype:'fieldset',
					title:'查询条件',
...
var cm =  new Ext.grid.ColumnModel({
	 ...
	 sm,
	 {
	 			...
	 			header: '工序系数',  sortable: true, dataIndex: 'DIFFICULTY_VALUE',renderer : function(value,metadata,record,rowIndex,colIndex,store ){
	 			...
	 			header: '调整系数',  sortable: true, dataIndex: 'DIFFICULTY_MODIFY_VALUE'
	 }]
});
...
var store = new Ext.data.Store({
		proxy : new  Ext.data.HttpProxy({
			 url : '../schedulingBase/processMgt_queryProcess.action'
		}),
							...
							fbar : [{
								...
								handler : function(){
										var form = this.ownerCt.ownerCt;
												...
												form.getForm().submit({
													url: '../schedulingBase/processMgt_saveProcess.action',
								...
								items : [{
										...
										fbar : [{
											...
											handler : function(){
													...
													if(form.getForm().isValid ()){
															form.getForm().submit({
																url: '../schedulingBase/processMgt_updateProcess.action',
																success: function(form, action) {
																	Ext.Msg.alert('提示', action.result.msg);
																	win.close();
																	queryProcess();
...
var exportProcess = function(){
		...
		myMask.show();
		Ext.Ajax.request({
			url : "../schedulingBase/processMgt_exportProcess.action",
			method : 'POST',
			params : {
				deptId : deptId,
...
var downTemp = function(){
		url = "uniquePart=6adf1b32-31f8-4125-91ec-b50ba39bc4d9admin&fileName=工序导入模板.xls&moduleName=schedulingBase&entityName=SchedulingBase&isTemplate=true";
		window.location =  '../common/downloadReportFile.action?' + encodeURI(encodeURI(url));
			...
			var win = new Ext.Window({
						...
						items : [{
								...
								items : [{
									...
									html : '<font size=3>模板文件： </font> <a href="#" onclick="downTemp()">工序代码导入模板下载</a>'
							}],
							fbar : [{
									text : '上传',
									handler :function(){
											...
											basicForm.submit({
												url: '../schedulingBase/processUploadFile.action',
												success: function(form, action) {
...
var confirmProcess = function(){
	//取当前用户所属网点的工序信息
	Ext.Ajax.request({
					url : '../schedulingBase/processMgt_queryUserProcess.action',
					method : 'POST',
					success : function(res,config){
	...
	var winShow = function (processObj){
			...
			var cm =  new Ext.grid.ColumnModel({
				 ...
				 new Ext.grid.RowNumberer({header : '序号',width : 35}),
				 {
			 			...
			 			renderer : function(value,metadata,record,rowIndex,colIndex,store){
				 			...
				 			header: '总部难度系数', align: 'center',width : 120,sortable: true, dataIndex: 'difficultyValue'
				 },{
				 			header: '难度系数修正',align: 'center', width : 100,sortable: true, dataIndex: 'difficultyModifyValue',
				 			editor :  new Ext.form.NumberField({minValue : 1,maxValue : 10,decimalPrecision:1,maxText  : '难度系数最大值为10',minText : '难度系数最小值为1'})
				 },{
				 			header: '系数取值', align: 'center',width : 100,sortable: true, dataIndex: 'VALUE',renderer : function(value,metadata,record,rowIndex,colIndex,store){
			...
			var store = new Ext.data.Store({
					proxy : new  Ext.data.HttpProxy({
						 url : '../schedulingBase/processMgt_findByDeptId.action'
					}),
					reader: new  Ext.data.JsonReader({
						root: 'root',
						totalProperty: 'totalSize'
			...
			var submitProcess = function(win){
					...
					store.each(function(r){
						...
						if(count==store.getTotalCount()){
									...
									Ext.Ajax.request({
												url : '../schedulingBase/processMgt_confirmProcess.action',
												method : 'POST',
												params : {
...
var  isOnlyProcess = function(key,value,obj){
		...
		params[key] = value;
		Ext.Ajax.request({
						url : '../schedulingBase/processMgt_isOnlyProcess.action',
						method : 'POST',
						params : params,
						success : function(res,config){
								var dataObj = Ext.decode(res.responseText);
...
var noticeStore = new Ext.data.JsonStore({
	url:"../schedulingBase/processMgt_pushMsg.action",
	totalProperty:'totalSize',
	root : 'root'
